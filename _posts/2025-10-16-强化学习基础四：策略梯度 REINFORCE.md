...
- 这里的对数梯度，并不是对策略网络输出的整个分布求梯度，而是对“在当前样本中所选动作的对数概率”求梯度，是一个标量。log_π(a|s) 的梯度意味着当前选中动作本身的概率梯度，表示如何调整参数能够增大或减小动作概率。而Q_π(s,a) 表示当前动作在当前状态下的期望回报。两者结合，使高回报的动作往增加概率的方向调整参数。

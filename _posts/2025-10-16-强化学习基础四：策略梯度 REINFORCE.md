...
value-based 方法算动作值时需要枚举动作，只适用于离散动作，对于连续动作的情况，比如机器人控制问题，发展出 policy-based 方法，1983年，Williams, Ronald J. 发表论文推导出策略梯度公式，提出REINFORCE算法。

policy-based方法绕开状态动作价值，通过迭代策略参数，直接优化策略，其核心思想是将策略参数化并优化策略本身。策略梯度的方法首先定义最优策略的目标函数，然后用基于梯度的优化算法来更新参数，这里策略参数化的实现方式可以是机器学习的各类模型，可以是浅度的线性函数，也可以是深度的神经网络，比较常用的是深度网络。

表格形式下，行列分别代表动作和状态，当状态动作空间较大，存储和索引压力都很大，而将策略参数化，能够提高效率和泛化性。

接下来的问题便是，如何定义目标函数，如何进行梯度优化，收敛性是否有保障？

与表格形式下的最优策略不同，参数化下的最优策略定义是能够最大化目标函数的策略。
